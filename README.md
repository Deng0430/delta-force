...
说明：“完成”表示当前进度表中的对应项目已经核对；“未标注”表示原进度表为空，不直接等同于未适配；“大事件”和“修改”表示仍有专项内容需要处理。临界点涉及的大事件不影响刷新载具、据点及占领区、攻守活动区等当前适配内容，因此计为完整适配。风暴眼守方在原表中另有“？？”备注，因此标记为待确认。游戏后续更新仍可能带来数据变化。
...
- 新增堑壕、临界点和断轨的完整胜者为王适配；其他地图的当前进度见上方表格。
...
- 新增 Markdown 部署备注，可为当前战术方案补充较完整的行动说明、分工和注意事项。
- 支持富文本复制与导出，方便将说明转移到飞书等支持 Markdown 或富文本内容的协作平台。
...
该项目是由社区开发者独立维护的衍生版本，其功能、数据兼容性、更新进度和安装包可能与本项目正式版本存在差异。如需体验联网功能，请前往该项目查看说明。联网功能尚未集成到本项目正式版。
